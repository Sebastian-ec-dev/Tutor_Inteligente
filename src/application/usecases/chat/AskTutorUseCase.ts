...
export class AskTutorUseCase {
  ...
  async execute(input: {
    ...
    if (!input.classId)
      throw new Error("Selecciona una clase/tema antes de preguntar");
    ...
    const notes = await this.audioNoteRepository.listBySubject(
      input.subjectId,
      userId,
    );
    ...
    const safeContext =
      contextText ||
      "No existe contexto procesado para la clase seleccionada todavía.";
    ...
    const model = this.modelRouter.selectModel(resolvedContentType);
