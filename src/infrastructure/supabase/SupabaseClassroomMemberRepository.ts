...
function isSchemaCacheError(error: { message?: string } | null): boolean {
  ...
  return message.includes('schema cache') || message.includes('relationship') || message.includes('Could not find');
}
...
export class SupabaseClassroomMemberRepository implements ClassroomMemberRepositoryPort {
  async listMembers(subjectId: string): Promise<ClassroomMember[]> {
    ...
    return (membersData || []).map((row: any) => mapMember(row, profilesById.get(row.user_id)));
  }
  ...
  async inviteMember(input: {
    ...
    if (legacyError) throw new Error(`${legacyError.message}. Ejecuta supabase/sql/002_fix_schema_cache_members_invites.sql para habilitar QR/enlace.`);
  ...
  async createJoinInvite(input: {
    ...
    if (error) {
      if (isSchemaCacheError(error)) {
        throw new Error('A tu base de datos le faltan columnas de invitación para QR/enlace. Ejecuta supabase/sql/002_fix_schema_cache_members_invites.sql en Supabase SQL Editor y vuelve a abrir la app.');
      }
  ...
  async joinByToken(token: string): Promise<{ subjectId: string; role: ClassroomRole; message: string }> {
    ...
    return {
      subjectId: row?.subject_id,
      role: row?.joined_role || 'student',
      message: row?.message || 'Te uniste correctamente al aula',
    };
  }
  ...
  async removeMember(input: { subjectId: string; memberUserId: string }): Promise<void> {
    const { error } = await supabase
      .from('subject_members')
      .delete()
      .eq('subject_id', input.subjectId)
